...
let displaySkorNaruto = document.getElementsByClassName("skor-naruto")[0];
...
if (localStorage.getItem("skorNaruto")) {
    skorNaruto = localStorage.getItem("skorNaruto");
    displaySkorNaruto.innerHTML = skorNaruto;
}
...
function janken(tangan) {
    let jariNaruto = Math.floor(Math.random() * 3);

    switch (jariNaruto) {
        case 0:
            naruto.style.backgroundImage = "url(assets/img/naruto-batu.png)";
            break;
        case 1:
            naruto.style.backgroundImage = "url(assets/img/naruto-kertas.png)";
            break;
        default:
            naruto.style.backgroundImage = "url(assets/img/naruto-gunting.png)";
            break;
    }

    naruto.classList.remove("goyang");
    
    switch (tangan) {
        case 0:
            if (jariNaruto == 0) {
                result("draw");
            } else if (jariNaruto == 1) {
                result("naruto");
            } else {
                result("player");
            }
...
function result(who) {
    ...
    switch (who) {
        case "naruto":
            skorNaruto++;
            localStorage.setItem("skorNaruto", skorNaruto);
            displaySkorNaruto.innerHTML = skorNaruto;
            console.log("Naruto Menang");
            break;
        case "player":
            skorPlayer++;
    ...
    timeOut = setTimeout(() => {
        naruto.style.removeProperty("background-image");
        naruto.classList.add("goyang");
    }, 3000);
}
